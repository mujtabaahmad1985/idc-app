(function($){
  ...
  */
  ...
  var inbox = {
    ...
    fileInboxList: "/cs-admin/messages/inbox",
    fileInboxMessage: "/cs-admin/messages/message",
    fileInboxReply: "/cs-admin/messages/compose",
    fileInboxListSent: "/cs-admin/messages/sent",
    fileInboxListSpam: "/cs-admin/messages/inbox",
    fileInboxListTrash: "/cs-admin/messages/inbox",
    fileInboxListStarred: "/cs-admin/messages/inbox",
        ...
        success: function(data, textStatus, jqXHR){
          ...
          $(document).on("click", ".message-reply-heading [data-action]", inbox.ccbcc);
            $(".send-message").click(function(e){
               if($("#componse_email").valid()){
                 $(".ajax-loader").show();
                   $("#componse_email").ajaxForm(function(response){
                       $(".ajax-loader").hide();
    ...
    fileUpload: function(){
      ...
      var myDropzone = new Dropzone(document.querySelector("#actions"), { // Make the whole body a dropzone
        url: "/cs-admin/message/attachment", // Set the url
        thumbnailWidth: 150,
        thumbnailHeight: 100,
        parallelUploads: 20,
      ...
      myDropzone.on("addedfile", function(file) {
        ...
        file.previewElement.querySelector(".start").onclick = function(e) {  myDropzone.enqueueFile(file); e.preventDefault(); };
      });
