...
function mg_news_pagination_rewrite() {
  add_rewrite_rule('/page/?([0-9]{1,})/?$', 'index.php?pagename='.get_option('category_base').'&paged=$matches[1]', 'top');
}
...
function chauffeur_theme_register_required_plugins() {
	...
	$plugins = array(
		...
		array(
			...
			'required' 				=> true, // If false, the plugin is only 'recommended' instead of required
			'version' 				=> '3.6.8', // E.g. 1.0.0. If set, the active plugin must be this version or higher, otherwise a notice is presented
			'force_activation' 		=> false, // If true, plugin is activated upon theme activation and cannot be deactivated until theme switch
			'force_deactivation' 	=> false, // If true, plugin is deactivated upon theme switch, useful for theme-specific plugins
			'external_url' 			=> '', // If set, overrides default API URL and points to an external URL
		),
		array(
			'name'     				=> esc_html__('Visual Composer','chauffeur'), // The plugin name
		...
		array(
			...
			'required' 				=> false, // If false, the plugin is only 'recommended' instead of required
			'version' 				=> '5.2.7', // E.g. 1.0.0. If set, the active plugin must be this version or higher, otherwise a notice is presented
			'force_activation' 		=> false, // If true, plugin is activated upon theme activation and cannot be deactivated until theme switch
			'force_deactivation' 	=> false, // If true, plugin is deactivated upon theme switch, useful for theme-specific plugins
			'external_url' 			=> '', // If set, overrides default API URL and points to an external URL
		),
...
if( ! function_exists( 'chauffeur_load_js' ) ) {
	function chauffeur_load_js() {
		...
		if ( is_admin() ) {
			...
			if( isset($chauffeur_data['main-color']) ) {
				$output .= '.header-area-1 .logo h2:before,
...
$wpccfid=$wpcf->id;
    // if you wanna check the ID of the Form $wpcf->id
     if ( '5' == $wpccfid ) { // Change 123 to the ID of the form  
echo '
