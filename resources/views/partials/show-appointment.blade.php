@if(in_array(39,$permissions_allowed) || Auth::user()->role=='hospital-administrator')
...
@endphp
@php
    $app = \App\Appointments::find($user_info['id']);
    $appointment_date = $app->booking_date.' '.date('H:i', strtotime($app->start_time));
	//dump($appointment_date);
    $ap_date = \Carbon\Carbon::parse($appointment_date);
    $c = strtotime($app->created_at);
    $now = time();
            ...
            <div class="list-icons">
                <div class="list-icons-item dropdown">
                    ...
                    <div class="dropdown-menu dropdown-menu-right" x-placement="bottom-end">
                        ...
                        @if(!$ap_date->isPast())
                        @if(in_array(46,$permissions_allowed) || Auth::user()->role=='hospital-administrator')
                        ...
                        @endif
                        @endif
                        <a href="#" class="dropdown-item appointment-action"  data-action="checkin"><i class="icon-cancel-square"></i> Check In </a>
                        @if(!$ap_date->isPast())
                    ...
                    {{--    <a href="/view/treatment-cards/{!! $user_info['patient_id'] !!}" class="dropdown-item"><i class="icon-book2"></i> Treatment Record</a>--}}
                        ...
                        <div class="dropdown-divider"></div>
                        <a href="/calendar/appointment/{!! $user_info['patient_id']  !!}/{!! strtolower(Illuminate\Support\Str::slug($user_info['patient_name'])) !!}" class="dropdown-item"><i class="icon-plus-circle2"></i> Make Appointment</a>
                        ...
                        <a href="#!" class="dropdown-item patient-actions"  data-action="contact-patient" data-patient-id="{!! \App\Helpers\CommonMethods::encrypt($user_info['patient_id'])  !!}"><i class="icon-address-book3"></i> Contact Patient </a>
...
<div class="card card-body">
    ...
    @if(isset( $app->appointment_activities) && $app->appointment_activities->count() > 0)
        <table class="table table-borderless">
            @foreach($app->appointment_activities as $activity)
                <tr>
                    @if($activity->user_id==1)
                    <td style="padding: 0">
                       <p class="small"><strong>Admin</strong> <span class="badge badge-primary">{!! $activity->status !!}</span>  appointment at {!! \App\Helpers\CommonMethods::formatDateTime($activity->created_at) !!}</p>
                    </td>
                        @else
                    <td style="padding: 0">
                        <p class="small"><strong>{!! $activity->users->name !!}</strong>  <span class="badge badge-primary">{!! $activity->status !!}</span>  appointment at {!! \App\Helpers\CommonMethods::formatDateTime($activity->created_at) !!}</p>
...
<div class="col s1 right"><a href="#!" class="grey-text close modal-action modal-close"></a>  </div>
    <label class="error">You have not granted for viewing appointment, contact hospital-administrator.</label>
    @endif