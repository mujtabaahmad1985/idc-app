...
<div id="header">
    ...
    <table border="0" style="width:700px">
        <tr>
            ...
            <td width="200px" style="text-align: left; width: 200px"><h1 style="font-size: 20px">Expense {!! $sales->expense_categories->name  !!}&nbsp;</h1> </td>
    ...
    <table border="0" style="width: 700px; margin-top: 10px">
        <tr>
            <td width="70%">
                <h3>{!! isset($sales->person_name)?$sales->person_name:"" !!}</h3>
                <p style="margin: 0">{!! isset($sales->email)?$sales->email:"" !!}</p>
                <p>{!! isset($sales->billing_address)?$sales->billing_address:"" !!}</p>
            ...
            <td width="30%" style="vertical-align: text-bottom; text-align: right">
                ...
                <ul style="list-style: none; margin: 0; padding: 0; line-height: 15px">
                    <li>Date : {!! \App\Helpers\CommonMethods::formatDate($sales->created_at) !!}</li>
                    <li>Invoice No. : {!! $sales->id !!}</li>
    ...
    <table border="1" style="width: 700px; margin-top: 10px; border-collapse: collapse;">
        <tr style="background: #000; color:#FFF">
            ...
            <th style="padding: 3px">Paid Price</th>

        </tr>
        @if(isset($sales->expense_items) && $sales->expense_items->count() > 0)
            @foreach($sales->expense_items as $item)
            <tr>
                <td style="padding: 3px">{!! $item->item_name !!}</td>
                <td style="padding: 3px">{!! $item->item_description !!}</td>
                <td style="padding: 3px">{!! $item->quantity !!}</td>
                <td style="padding: 3px">{!! number_format($item->price,2) !!}</td>
                <td style="padding: 3px">0</td>
                <td  style="padding: 3px">{!! number_format($item->total_price,2) !!}</td>
            </tr>
            @endforeach
            @endif
    </table>
