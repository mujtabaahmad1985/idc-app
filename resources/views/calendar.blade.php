...
<style>
	...
	.fc-event+.fc-event{ margin-top:0 !important}
    .fc-icon:after {
        ...
        <div class="card calendar-card" style="margin-bottom: 57px">
            ...
            <div class="row">
                ...
                <div class="col-xl-7">
                    ...
                    <div class="row">
                        @if((in_array(10,$permissions_allowed) || Auth::user()->role=='hospital-administrator') )
                        <div class="col-md-4">
                            ...
                            <select class="form-control-sm select2" id="filter-doctors" name="doctors">
                                ...
                                @if(isset($doctors) && count($doctors) > 0)
                                    ...
                                    @foreach($doctors as $doctor)
                                        <option value="{!! $doctor->id !!}" @if(Session::get('search_doctor_id')==$doctor->id) selected @endif>{!! $doctor->fname.' '.$doctor->lname !!}</option>
                                    @endforeach
                                @endif
                            </select>
                        ...
                        @endif
                            @if((in_array(11,$permissions_allowed) || Auth::user()->role=='hospital-administrator') )
                        <div class="col-md-4">
                            <div class="input-group">
<span class="input-group-prepend">
                                ...
                                <select class="form-control-sm select2"  name="locations" >
                                    ...
                                    @if(isset($locations) && count($locations) > 0)
                                        ...
                                        @foreach($locations as $location)
                                            <option value="{!! $location->id !!}" @if(Session::get('search_location_id')==$location->id) selected @endif>{!! $location->name !!}</option>
                                        @endforeach
                                    @endif
                                </select>
                                ...
                                <select class="form-control-sm select2" name="services">
                                    ...
                                    @if(isset($services) && count($services) > 0)
                                        ...
                                        @foreach($services as $service)
                                            <option class=""  value="{!! $service->id !!}"  @if(Session::get('search_service_id')==$service->id) selected @endif   data-duration="{!! date('H:i', strtotime($service->duration)) !!}">{!! substr($service->service_name,0,25) !!}</option>
                                        @endforeach
                                    @endif
                                </select>
                            </div>
                ...
                <div class="col-xl-5 text-right appointment-button-filters">
                    <div class="btn-group">
                        ...
                        @if((in_array(8,$permissions_allowed) || Auth::user()->role=='hospital-administrator') )
                        <button type="button" class="btn btn-danger calendar-options btn-sm"  data-option="room"> Room</button>
                        @endif
                            @if((in_array(9,$permissions_allowed) || Auth::user()->role=='hospital-administrator') )
                    ...
                    <div class="btn-group">
                        @if(in_array(6,$permissions_allowed) || Auth::user()->role=='hospital-administrator')
                        <button type="button" class="btn btn-danger add-appointment btn-sm">Book Appointment</button>
                        @endif
                            @if((in_array(45,$permissions_allowed) || Auth::user()->role=='hospital-administrator') )
        ...
        <div class="card-body">
            <div class="row text-center">
                ...
                <div class="alert bg-danger text-white text-center" style="display: block">
                    ...
                    <span class="font-weight-semibold">Oh snap!</span> You are not authorized to this page, please <a href="#" class="alert-link">contact hospital-administrator</a>.
                </div>
            </div></div>
        </div>
    </div>
...
{{--    {!! Html::script('public/material/js/plugins/fullcalendar/js/fullcalendar.min.js') !!}--}}
    ...
    <script>
        var calendar = null;
        var start_calendar_time = "07:00:00";
        ...
        var doctor_ids = [{!! implode(',',$doctors->pluck('id')->all()) !!}];
...
//alert();
                            ...
                            switch(options){
                                ...
                                case "room":
                                    ...
                                    $("#calendar").fullCalendar({
                                        ...
                                        header: {
                                            left: 'prev,next today,default_calendar,google_calendar',
                                            center: 'title',
                                        ...
                                        customButtons: {
                                            ...
                                            google_calendar_hide: {
                                                text: "Hide GCalendar's App",
                                                click: function() {
                                                    // $("#calendar").fullCalendar('destroy');
                                                    // set_calendar($("#calendar"));
                                                    // $("#calendar").fullCalendar('changeView', 'agendaWeek');
                                                    ...
                                                    $(doc).each(function(i,v) {
                                                        if(v.appointment_type=='appointment'){
                                                            ...
                                                            events.push({
                                                                ...
                                                                resourceIds:(v.doctor_id?[v.doctor_id]:doctor_ids),
                                                                color:v.color,
                                                                status:v.status,
          ...
          //  $('#payment-table').editableTableWidget();
            ...
            calendar =  $("#calendar").fullCalendar({
                ...
                header: {
                    left: 'prev,next today,default_calendar,google_calendar,google_calendar_hide',
                    center: 'title',
                    right: 'agendaDay,agendaTwoDay,agendaWeek,month'
                },
                customButtons: {
                    ...
                    google_calendar: {
                        text: 'Appointments From G Calendar',
                        click: function() {
                            // $("#calendar").fullCalendar('destroy');
                            // set_calendar($("#calendar"));
                            // $("#calendar").fullCalendar('changeView', 'agendaWeek');
                   ...
                   /* $.ajax({
                    ...
                    $("#filter-appointment").ajaxForm(function(doc){
                     ...
                     //   console.log(google_events);
                        $(google_events).each(function (i,v) {
                            events.push({
                                id: 'google-'+v.id,
                                title: v.title,
                                start: v.start, // will be parsed
                ...
                eventClick:function(event, jsEvent, view){
                    ...
                    $.ajax({
                        url:"/get/google/appointment/detail",
                        type:"POST",
                        data:{
                            id:event.id,
                            '_token':'{!! csrf_token() !!}',
                        ...
                        success:function (response) {
                            $("#show-appointment").html(response);
            ...
            $("body").on('click','.fc-google_calendar-button',function () {
                gapi.auth2.getAuthInstance().signIn();
            });

            $("body").on('click','.fc-google_calendar_hide-button',function () {
                ...
                dayClick: function (date, jsEvent, view,resource) {
                   ...
                   // var selected_date =  d.getDate()+"."+(d.getMonth()+1)+"."+d.getFullYear();
                    ...
                    $.ajax({
                        url:"/make/appointment",
                        type:"POST",
                        data:{"_token":"{!! csrf_token() !!}",strTime:strTime,selected_date:selected_date, patient_id:patient_id, patient_name:patient_name,room_id:resource.id,doctor_id:doctor_id,un_available_doctor_id:un_available_doctor_id,location_id:location_id},
                        success:function (response) {
                            var calendar = calendar;
                            $("#make-appointment").html(response);
    ...
    <script type="text/javascript">
        ...
        var apiKey = 'AIzaSyCx6Q2l_ZlcJEGFAwBHlFBbNhYiFP8_L2o';
        ...
        var clientId = '607102180423-riunepkvj216jm11qjes9rtbupeml74n.apps.googleusercontent.com';
        // Enter one or more authorization scopes. Refer to the documentation for
        // the API or https://developers.google.com/people/v1/how-tos/authorizing
        // for details.
        var scopes = 'https://www.googleapis.com/auth/calendar.readonly';
        var authorizeButton = $(".fc-google_calendar-button");
        var signoutButton =  $(".fc-google_calendar_hide-button");
        ...
        function updateSigninStatus(isSignedIn) {
            if (isSignedIn) {
                $(".fc-google_calendar-button").hide();
                $(".fc-google_calendar_hide-button").show();
                listUpcomingEvents();
            }else{
                $(".fc-google_calendar-button").show();
                $(".fc-google_calendar_hide-button").hide();
            }
        }
            ...
            gapi.client.calendar.events.list({
                ...
                if (events.length > 0) {
                    ...
                    for (i = 0; i < events.length; i++) {
                        ...
                        var start_time = event.start.dateTime.split('+');
                        var end_time = event.end.dateTime.split('+');
